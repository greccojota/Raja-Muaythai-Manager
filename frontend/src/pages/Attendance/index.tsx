...
export function AttendancePage() {
  ...
  const { data: attendanceData, isLoading } = useQuery({
    queryKey: ['attendance', page, filterStudent, filterClass, filterDateFrom, filterDateTo],
    queryFn: () =>
      attendanceService.list({
        page: page + 1,
        size: 50,
        student_id: filterStudent || undefined,
        class_group_id: filterClass || undefined,
        date_from: filterDateFrom || undefined,
        date_to: filterDateTo || undefined,
      }),
  })
  const { data: frequency } = useQuery({
    queryKey: ['attendance-frequency'],
    queryFn: () => attendanceService.getFrequency(),
    enabled: tab === 1,
  })
  const { data: students } = useQuery({
    queryKey: ['students-all'],
    queryFn: () => studentService.list({ size: 200 }),
  })
  const { data: classes } = useQuery({
    queryKey: ['classes-active'],
    queryFn: () => classesService.listGroups(true),
  })
  ...
  const hasFilters = filterStudent || filterClass || filterDateFrom || filterDateTo
  const resetFilters = () => {
    setFilterStudent('')
    setFilterClass('')
    setFilterDateFrom('')
  ...
  return (
    <Box>
      <PageHeader
        ...
        subtitle="Historico de sessoes realizadas e frequencia"
        actionLabel="Registrar aula"
        actionIcon={<SessionIcon />}
        onAction={() => setSessionOpen(true)}
      />
        ...
        <Box>
          <Box sx={{ display: 'flex', gap: 1, mb: 1.5, flexWrap: 'wrap', alignItems: 'center' }}>
            <TextField
              ...
              size="small"
              value={filterStudent}
              onChange={e => { setFilterStudent(e.target.value); setPage(0) }}
              sx={{ width: 200 }}
            >
      ...
      {tab === 1 && (
        <Box>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Total de presencas por aluno no mes corrente e historico geral.
          </Typography>
          <Grid container spacing={2}>
            {(frequency ?? []).map((f: AttendanceSummary) => (
              <Grid item xs={12} sm={6} md={4} key={f.student_id}>
                <Card
                  ...
                  onClick={() => navigate(`/students/${f.student_id}`)}
                >
                  <CardContent>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                      <Box sx={{ textAlign: 'center' }}>
                        <Typography variant="h5" fontWeight={800} color="primary.main">{f.checkins_this_month}</Typography>
                        <Typography variant="caption" color="text.secondary">Este mes</Typography>
                      </Box>
                      <Box sx={{ textAlign: 'center' }}>
                        <Typography variant="h5" fontWeight={800}>{f.total_checkins}</Typography>
                        <Typography variant="caption" color="text.secondary">Total</Typography>
                      </Box>
                      <Box sx={{ textAlign: 'center' }}>
                        <Typography variant="caption" color="text.secondary" display="block">Ultimo</Typography>
                        <Typography variant="caption">{formatDate(f.last_checkin?.slice(0, 10))}</Typography>
                      </Box>
                    </Box>
                  </CardContent>
                </Card>
            ...
            {(frequency ?? []).length === 0 && (
              <Grid item xs={12}>
                <Alert severity="info">Nenhuma aula registrada ainda.</Alert>
              </Grid>
            )}
          </Grid>
        </Box>
      ...
      <Dialog open={sessionOpen} onClose={() => setSessionOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: '16px !important' }}>
          ...
          <TextField
            ...
            onChange={e => setForm(p => ({ ...p, class_group_id: e.target.value }))}
          >
            <MenuItem value="">Livre / sem turma</MenuItem>
            {(classes ?? []).map(c => <MenuItem key={c.id} value={c.id}>{c.name}</MenuItem>)}
          </TextField>
          <TextField
            label="Observacoes"
      ...
      <ConfirmDialog
        ...
        message={`Remover registro de aula de "${deleteTarget?.student_name}"?`}
